<template>
  <div class="c-swipe">
    <div class="c-swipe-items" ref="wrap">
      <slot></slot>
    </div>
    <div class="c-swipe-indicators" v-show="showIndicators">
      <div class="c-swipe-indicator" v-for="(key, page) in pages" :class="{ active: key === index }"></div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    created () {
      this.dragState = {}
      this.bus.$on('swipeItemCreated', this.swipeItemCreated)
      this.bus.$on('swipeItemDestroyed', this.swipeItemDestroyed)
    },
